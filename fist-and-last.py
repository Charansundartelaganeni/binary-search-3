...
class Solution:
    def searchRange(self, nums: List[int], target: int) -> List[int]:
        self.nums = nums
        self.target = target
        first = self.binarySearch(True) #By setting first flag value to True the binary search is performed on the left side of the array to find the first position of the target
        last = self.binarySearch(False) #By setting last flag value to False the binary search is performed on the right side of the array to find the last position of the target
        ...
        while low <= high:  #Loop over the array till low becomes less than or equal to high
            ...
            if self.nums[mid] == self.target:   #If the element in mid index is equal to the target then change index value to mid
                ...
                if leftFlag:    #If leftFlag is true then the binarySearch is done for the left side of the array
                    ...
                    low = mid+1 #Else change the low to mid+1, operation is performed on the right side
